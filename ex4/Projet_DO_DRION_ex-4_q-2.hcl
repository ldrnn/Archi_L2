...
intsig LOOPE                    'instructionSet.get("loope").icode'
intsig LOOPNE                   'instructionSet.get("loopne").icode'
...
intsig RECX                     'registers.ecx'         # Loop counter register (%ecx)
...
bool need_valC =
    icode in { IRMOVL, RMMOVL, MRMOVL, JXX, CALL, IOPL, LOOP, LOOPE, LOOPNE };

## Is instruction valid?
bool instr_valid =
    icode in { NOP, HALT, RRMOVL, IRMOVL, RMMOVL, MRMOVL,
               OPL, IOPL, JXX, CALL, RET, PUSHL, POPL, LOOP, LOOPE, LOOPNE };
...
int srcB = [
    icode in { LOOP, LOOPE, LOOPNE } : RECX; 
...
int aluB = [
    icode in { RMMOVL, MRMOVL, OPL, IOPL, CALL, PUSHL, RET, POPL, LOOP, LOOPE, LOOPNE } : valB;
...
bool is_bch = icode in { JXX, LOOP, LOOPE, LOOPNE };
...
int new_pc = [
    # LOOP, LOOPE, LOOPNE
    (icode in { LOOP, LOOPE, LOOPNE } && valE != 0 && (icode == LOOP || Bch)) : valC;
